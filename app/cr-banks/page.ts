...
import { RefreshCw, Download, Search, Filter, AlertTriangle, TrendingUp } from 'lucide-react'
...
export default function CRBanksDashboard() {
    ...
    if (amountFilter !== 'all') {
      switch (amountFilter) {
        ...
        case 'small':
          ...
          break
        case 'over10k':
          filtered = filtered.filter(t => t.amount >= 10000)
          break
        case 'over50k':
  ...
  }, [data, searchTerm, selectedIssuer, amountFilter, dateFilter, selectedMonth, selectedYear])
  ...
  const clearFilters = () => {
    ...
    setAmountFilter('all')
    setDateFilter('month') // Reset to last 30 days instead of 'all'
